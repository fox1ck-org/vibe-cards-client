...
func TestNamedErrorsAreDistinguishable(t *testing.T) {
	cases := []struct {
		...
		{"closed card", 400,
			...
			ErrNotBindable},
		{"quarantined card", 400,
			`{"code":"failed_precondition","message":"card is unreadable: the provider shows its number without an expiry or cvv, so it is quarantined — return it and draw another"}`,
			ErrUnreadable},
